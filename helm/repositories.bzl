...
load("//helm/private:versions.bzl", "TOOL_VERSIONS")
...
_DOC = "TODO"
_ATTRS = {
    "helm_version": attr.string(mandatory = True, values = TOOL_VERSIONS.keys()),
    "platform": attr.string(mandatory = True, values = PLATFORMS.keys()),
}

def _helm_repo_impl(repository_ctx):
    url = "https://github.com/someorg/someproject/releases/download/v{0}/helm-{1}.zip".format(
        repository_ctx.attr.helm_version,
        repository_ctx.attr.platform,
    )
    repository_ctx.download_and_extract(
        url = url,
        integrity = TOOL_VERSIONS[repository_ctx.attr.helm_version][repository_ctx.attr.platform],
    )
...
helm_toolchain(name = "helm_toolchain", target_tool = select({
        "@bazel_tools//src/conditions:host_windows": "helm_tool.exe",
        "//conditions:default": "helm_tool",
    }),
)
"""
...
def helm_register_toolchains(name, **kwargs):
    ...
    Args:
        name: base name for all created repos, like "helm1_14"
